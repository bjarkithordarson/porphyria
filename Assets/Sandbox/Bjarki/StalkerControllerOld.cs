...
public class StalkerControllerOld: MonoBehaviour
{
    ...
    public Vector3 sprintDestination;
    ...
    void Start()
    {
        ...
        sprintDestination = gameObject.transform.position;
    }
    ...
    void SpawnRandomly()
    {
        try
        {
            Vector3 position = GetRandomPositionInRadius(player.transform.position, spawnRadius);
            Debug.Log(position);
    ...
    {
        sprintDestination = position;
    }

    void Move()
    {
        while(gameObject.transform.position != sprintDestination)
        {
            gameObject.transform.position = Vector3.Lerp(gameObject.transform.position, sprintDestination, 0.1f);
        }
    }
